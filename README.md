...
`reviewQueue` 已接入国内电商意图的非敏感元数据，用于人工审核台排序和提示，不保存 `buyerMessage`、prompt、KB context、订单、客户资料或平台原始 payload。
...
```js
{
  ...
  risk_level: "MEDIUM",
  priority: "HIGH",
  review_note: "投诉/差评风险 / 优先处理"
}
```

优先级规则：

- `HIGH`：`COMPLAINT_RISK` / `FORBIDDEN_ACTION` / `ORDER_SENSITIVE`
- `MEDIUM`：`LOGISTICS` / `AFTER_SALE`
- `LOW`：`PRE_SALE` / `UNKNOWN`

审核列表按 `priority` 再按 `created_at` 排序：高优先级先处理，同优先级按创建时间先后处理。

固定审核提示：

- `FORBIDDEN_ACTION`：禁止承诺 / 必须人工核实
- `COMPLAINT_RISK`：投诉/差评风险 / 优先处理
- `ORDER_SENSITIVE`：订单/支付/地址/手机号敏感信息 / 人工核实

本阶段仍然不接真实抖店 / 淘宝 API，不自动发送任何回复，不调用真实订单、物流、退款或退货接口。

## 第四阶段：审核台 API

审核台接口只返回审核所需的非敏感元数据，不返回买家原文、prompt、KB context、raw context、订单、物流、支付或客户个人信息。

### 审核列表

```http
GET /api/v1/reviews?status=PENDING&priority=HIGH
X-API-Key: demo-secret-key
```
...
```text
HIGH -> MEDIUM -> LOW，再按 created_at 升序
```
...
- `COMPLAINT_RISK`：投诉/差评风险，需要优先人工安抚
